...
This fails before launch when persistent `debug.rustyquest.makepad.*` projection properties
...
- passthrough-only reference: same launch context, but
  ...
  `rustyquest.makepad.openxrPassthroughProbe=underlay`, and
  `rustyquest.makepad.mediaProjection=true`.
...
`rustyquest.makepad.directCamera2OesProjectionGeometryProfile=full-frame-diagnostic` and
the custom case with
`rustyquest.makepad.directCamera2OesProjectionGeometryProfile=camera-projection`. The
...
powershell -ExecutionPolicy Bypass -File .\tools\quest-camera-profile\Invoke-QuestCameraProfileRun.ps1 `
  -Serial <serial> `
  -RuntimeProfile camera-stereo-gpu-composite-performance-065 `
  -Override 'rustyquest.makepad.cameraTargetFps=0,rustyquest.makepad.cameraStereoImageReaderMaxImages=3' `
  -CaptureHzdbScreencap
```
...
properties. Stale `debug.rustyquest.makepad.projection.*` projection properties are
...
`debug.rustyquest.makepad.direct.camera.hardware.buffer.external` explicitly and
...
`-Override 'rustyquest.makepad.openxrPassthroughProbe=warmup'` or
`-Override 'rustyquest.makepad.openxrPassthroughProbe=client'`. Use
...
powershell -ExecutionPolicy Bypass -File .\tools\quest-camera-profile\Invoke-QuestCameraProfileRun.ps1 `
  -Serial <serial> `
  -RuntimeProfile camera-stereo-gpu-composite-native-ndk-065 `
  -Override 'rustyquest.makepad.cameraStartDelayMs=2500,rustyquest.makepad.nativeSourceMode=delayed-synthetic-dual-back' `
  -CaptureHzdbScreencap
```
...
powershell -ExecutionPolicy Bypass -File .\tools\quest-camera-profile\Invoke-QuestCameraProfileRun.ps1 `
  -Serial <serial> `
  -RuntimeProfile camera-stereo-gpu-composite-native-single-mirror-065 `
  -Override 'rustyquest.makepad.nativeLeftCameraId=<camera-id>' `
  -CaptureHzdbScreencap
```
