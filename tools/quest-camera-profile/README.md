...
Source sampling is separate from projection mode. Use
`-SourceSamplingMode target-local-raster` when the test is "place this
incoming raster in the target footprint"; use
`-SourceSamplingMode screen-to-camera-homography` when the test is the
...
direct Camera2 or broker H.264 metadata target rect is authored to match the
current bounded screen-to-camera homography footprint.
...
| `rustyxr.projectionTargetOffsetXUv` / `rustyxr.projectionTargetOffsetYUv` | float | Runtime target-footprint offset applied after source metadata in display-eye screen UV. This moves the target-local raster footprint without changing the source metadata contract. |
| `rustyxr.projectionTargetScale` | float | Runtime target-footprint scale applied around the metadata/fallback target center. Values below `1.0` shrink the coherent target footprint and increase the peripheral-stretch exterior. |
| `rustyxr.projectionTargetJoystickControls` | string | HWB OpenXR controller mode for runtime target-footprint adjustment. `offset-scale` maps left stick to target offset, right stick Y to target scale, and right A to reset. |
| `rustyxr.brokerH264LeftTargetScreenUvRect` / `rustyxr.brokerH264RightTargetScreenUvRect` | string | Broker H.264 per-eye target-footprint overrides, encoded as `x,y,width,height`; these are forwarded into broker stream metadata so HWB/OES consume the same target-local raster contract as direct Camera2. |
