...
param(
    ...
    [double]$GlesBrokerProjectionAreaOffsetXUv = [double]::NaN,
    [double]$MakepadDirectProjectionAreaOffsetXUv = [double]::NaN,
    [double]$MakepadBrokerProjectionAreaOffsetXUv = [double]::NaN,
    [double]$FullFrameBrokerProjectionAreaOffsetXUv = [double]::NaN,
    [double]$VulkanFullFrameBrokerProjectionAreaOffsetXUv = [double]::NaN,
    [double]$GlesFullFrameBrokerProjectionAreaOffsetXUv = [double]::NaN,
    [double]$MakepadFullFrameBrokerProjectionAreaOffsetXUv = [double]::NaN,
    ...
    [double]$GlesCameraPreviewFovYDegrees = [double]::NaN,
    [double]$GlesCameraPreviewOffsetYMeters = [double]::NaN,
    [double]$GlesCameraRawOverlayOverscan = [double]::NaN,
    [double]$MakepadCameraPreviewFovYDegrees = [double]::NaN,
    [double]$MakepadCameraPreviewOffsetYMeters = [double]::NaN,
    [double]$MakepadCameraRawOverlayOverscan = [double]::NaN,
    ...
    [double]$VulkanProjectionDepthMeters = [double]::NaN,
    [double]$VulkanDirectProjectionDepthMeters = [double]::NaN,
    [double]$VulkanBrokerProjectionDepthMeters = [double]::NaN,
    [double]$GlesProjectionDepthMeters = [double]::NaN,
    [double]$GlesDirectProjectionDepthMeters = [double]::NaN,
    [double]$GlesBrokerProjectionDepthMeters = [double]::NaN,
    [double]$MakepadProjectionDepthMeters = [double]::NaN,
    [double]$MakepadDirectProjectionDepthMeters = [double]::NaN,
    [double]$MakepadBrokerProjectionDepthMeters = [double]::NaN,
...
function Get-StateSnapshotSummary {
    ...
    $adbStatePath = Join-Path $snapshotRoot "adb-get-state.txt"
    ...
    $adbState = if (Test-Path -LiteralPath $adbStatePath) { ((Get-Content -Raw -Path $adbStatePath) -split "`r?`n" | Select-Object -First 1).Trim() } else { "" }
...
function Get-StateIssueSummary {
    ...
    if ($Before.adbState -and $Before.adbState -ne "device") {
        $issues.Add(("before ADB state is {0}" -f $Before.adbState))
    }
    if ($After.adbState -and $After.adbState -ne "device") {
        $issues.Add(("after ADB state is {0}" -f $After.adbState))
    }
    if ($After.wakefulness -eq "Asleep") {
        $issues.Add("after wakefulness is Asleep")
    }
    if ($After.vrState -match '^(STANDBY|HEADSET_UNMOUNTED|WAITING_FOR_SLEEP_MSG)$') {
        $issues.Add(("after VR power state is {0}" -f $After.vrState))
    }
    ...
    if ($Before.virtualProximityState -and $After.virtualProximityState -and $Before.virtualProximityState -ne $After.virtualProximityState) {
        $issues.Add(("virtual proximity changed {0}->{1}" -f $Before.virtualProximityState, $After.virtualProximityState))
    }
...
function Get-StateReadinessIssue {
    param([object]$State)

    $issues = [System.Collections.Generic.List[string]]::new()
    if (-not $State.adbState) {
        $issues.Add("ADB state unavailable")
    }
    elseif ($State.adbState -ne "device") {
        $issues.Add(("ADB state is {0}" -f $State.adbState))
    }
    if ($State.wakefulness -and $State.wakefulness -ne "Awake") {
        $issues.Add(("wakefulness is {0}" -f $State.wakefulness))
    }
    elseif (-not $State.wakefulness) {
        $issues.Add("wakefulness unavailable")
    }
    if ($State.vrState -match '^(STANDBY|HEADSET_UNMOUNTED|WAITING_FOR_SLEEP_MSG)$') {
        $issues.Add(("VR power state is {0}" -f $State.vrState))
    }
    elseif (-not $State.vrState) {
        $issues.Add("VR power state unavailable")
    }
    return ($issues -join "; ")
}
...
function Add-PeripheralStretchRuntimeValues {
    ...
    $Values.Add("rustyquest.peripheralStretchMode=$PeripheralStretchMode")
    $Values.Add(("rustyquest.peripheralStretchCoreScale={0}" -f (Format-InvariantDouble -Value $PeripheralStretchCoreScale)))
    $Values.Add(("rustyquest.peripheralStretchEdgeInsetUv={0}" -f (Format-InvariantDouble -Value $PeripheralStretchEdgeInsetUv)))
    $Values.Add(("rustyquest.peripheralStretchMaxInsetUv={0}" -f (Format-InvariantDouble -Value $PeripheralStretchMaxInsetUv)))
    $Values.Add(("rustyquest.peripheralStretchCurve={0}" -f (Format-InvariantDouble -Value $PeripheralStretchCurve)))
    $Values.Add(("rustyquest.peripheralStretchInnerBlendUv={0}" -f (Format-InvariantDouble -Value $PeripheralStretchInnerBlendUv)))
    $Values.Add(("rustyquest.peripheralStretchBlendCurve={0}" -f (Format-InvariantDouble -Value $PeripheralStretchBlendCurve)))
    $Values.Add("rustyquest.peripheralStretchBlendMode=$PeripheralStretchBlendMode")
    $Values.Add("rustyquest.peripheralStretchCornerMode=$PeripheralStretchCornerMode")
    $Values.Add("rustyquest.peripheralStretchDebug=$PeripheralStretchDebug")
}

function Resolve-ProjectionAreaOffsetXUv {
    param([double]$RendererValue)
    if ([double]::IsNaN($RendererValue)) {
        return $ProjectionAreaOffsetXUv
    }
    return $RendererValue
}

function Resolve-ModeProjectionAreaOffsetXUv {
    param(
    ...
    return (Resolve-ProjectionAreaOffsetXUv -RendererValue $RendererValue)
}

function Resolve-BrokerModeProjectionAreaOffsetXUv {
    param(
        [double]$RendererValue,
        [double]$ModeValue,
    ...
    if ((Resolve-BrokerH264ProjectionGeometryProfile) -eq "full-frame-diagnostic") {
        ...
        if (-not [double]::IsNaN($FullFrameBrokerProjectionAreaOffsetXUv)) {
            return $FullFrameBrokerProjectionAreaOffsetXUv
        }
    }
    return (Resolve-ModeProjectionAreaOffsetXUv -RendererValue $RendererValue -ModeValue $ModeValue)
}
...
function Resolve-VulkanProjectionDepthMeters {
    param([double]$ModeValue)
    if (-not [double]::IsNaN($ModeValue)) {
        return $ModeValue
    }
    if (-not [double]::IsNaN($VulkanProjectionDepthMeters)) {
        return $VulkanProjectionDepthMeters
    }
    return $ProjectionDepthMeters
}

function Resolve-GlesProjectionDepthMeters {
    param([double]$ModeValue)
    if (-not [double]::IsNaN($ModeValue)) {
        return $ModeValue
    }
    if (-not [double]::IsNaN($GlesProjectionDepthMeters)) {
        return $GlesProjectionDepthMeters
    }
    return $ProjectionDepthMeters
}

function Resolve-MakepadProjectionDepthMeters {
    param([double]$ModeValue)
    if (-not [double]::IsNaN($ModeValue)) {
        return $ModeValue
    }
    if (-not [double]::IsNaN($MakepadProjectionDepthMeters)) {
        return $MakepadProjectionDepthMeters
    }
    return $ProjectionDepthMeters
}
...
function Resolve-VulkanOptionalDouble {
    ...
    if (-not [double]::IsNaN($CommonValue)) {
        return $CommonValue
    }
    return [double]::NaN
}

function Resolve-CommonOptionalDouble {
    param(
        [double]$CommonValue,
        [double]$RendererValue
    )
...
function Get-VulkanProjectionBorderOverride {
    param(
        [double]$OffsetXUv = (Resolve-ProjectionAreaOffsetXUv -RendererValue $VulkanProjectionAreaOffsetXUv),
        ...
        [double]$TargetProjectionDepthMeters = $ProjectionDepthMeters,
    ...
    $projectionDepth = Format-InvariantDouble -Value $TargetProjectionDepthMeters
    ...
    $commonValues.Add("rustyquest.cameraProjectionGeometryProfile=$CameraProjectionGeometryProfile")
    $commonValues.Add("rustyquest.cameraProjectionScale=$projectionScale")
    $commonValues.Add("rustyquest.projectionDepthMeters=$projectionDepth")
    $commonValues.Add("rustyquest.projectionAreaScaleUv=$projectionAreaScaleUv")
    $commonValues.Add("rustyquest.projectionAreaOffsetXUv=$offsetX")
    $commonValues.Add("rustyquest.projectionAreaOffsetYUv=$offsetY")
    $commonValues.Add("rustyquest.projectionAreaLeftOffsetXUv=$leftOffsetX")
    $commonValues.Add("rustyquest.projectionAreaLeftOffsetYUv=$leftOffsetY")
    $commonValues.Add("rustyquest.projectionAreaRightOffsetXUv=$rightOffsetX")
    $commonValues.Add("rustyquest.projectionAreaRightOffsetYUv=$rightOffsetY")
    $commonValues.Add("rustyquest.projectionAreaRadiusXUv=$areaRadiusX")
    $commonValues.Add("rustyquest.projectionAreaRadiusYUv=$areaRadiusY")
    $commonValues.Add("rustyquest.projectionAreaCornerRadiusUv=$areaCornerRadius")
    $commonValues.Add("rustyquest.projectionAreaOpacity=$areaOpacity")
    $commonValues.Add("rustyquest.projectionBorderOpacity=$borderOpacity")
    $commonValues.Add("rustyquest.projectionAlphaMode=$ProjectionAlphaMode")
    $commonValues.Add("rustyquest.projectionAlphaScale=$alphaScale")
    $commonValues.Add("rustyquest.projectionAlphaBias=$alphaBias")
    $commonValues.Add("rustyquest.processingLayer=$ProcessingLayer")
    $commonValues.Add("rustyquest.cameraBlurRadiusPx=$blurRadius")
    Add-PeripheralStretchRuntimeValues -Values $commonValues
    if (-not [double]::IsNaN($CameraPreviewFovYDegrees)) {
        $commonValues.Add(("rustyquest.cameraPreviewFovYDegrees={0}" -f (Format-InvariantDouble -Value $CameraPreviewFovYDegrees)))
    }
    if (-not [double]::IsNaN($CameraPreviewOffsetYMeters)) {
        $commonValues.Add(("rustyquest.cameraPreviewOffsetYMeters={0}" -f (Format-InvariantDouble -Value $CameraPreviewOffsetYMeters)))
    }
    if (-not [double]::IsNaN($CameraRawOverlayOverscan)) {
        $commonValues.Add(("rustyquest.cameraRawOverlayOverscan={0}" -f (Format-InvariantDouble -Value $CameraRawOverlayOverscan)))
    }
    ...
    return "rustyquest.makepad.cameraPipelinePreset=raw-projection-unorm,rustyquest.makepad.cameraProjectionEffectMode=raw-projection,rustyquest.projectionBorderPolicy=$ProjectionBorderPolicy,$passthroughOverride,$commonOverride"
}

function Get-GlesProjectionBorderOverride {
    param(
        [double]$OffsetXUv = (Resolve-ProjectionAreaOffsetXUv -RendererValue $GlesProjectionAreaOffsetXUv),
    ...
    $values.Add("rustyquest.cameraProjectionGeometryProfile=$CameraProjectionGeometryProfile")
    $values.Add("rustyquest.makepad.directCamera2OesProjectionGeometryProfile=$CameraProjectionGeometryProfile")
    $values.Add("rustyquest.projectionBorderPolicy=$ProjectionBorderPolicy")
    $values.Add("rustyquest.processingLayer=$ProcessingLayer")
    $values.Add("rustyquest.cameraBlurRadiusPx=$blurRadius")
    Add-PeripheralStretchRuntimeValues -Values $values
    $values.Add("rustyquest.projectionDepthMeters=$projectionDepth")
    $values.Add("rustyquest.projectionAreaOffsetXUv=$offsetX")
    $values.Add("rustyquest.projectionAreaOffsetYUv=$offsetY")
    $values.Add("rustyquest.projectionAreaLeftOffsetXUv=$leftOffsetX")
    $values.Add("rustyquest.projectionAreaLeftOffsetYUv=$leftOffsetY")
    $values.Add("rustyquest.projectionAreaRightOffsetXUv=$rightOffsetX")
    $values.Add("rustyquest.projectionAreaRightOffsetYUv=$rightOffsetY")
    $values.Add("rustyquest.projectionAreaScaleUv=$scaleUv")
    $values.Add("rustyquest.projectionAreaRadiusXUv=$areaRadiusX")
    $values.Add("rustyquest.projectionAreaRadiusYUv=$areaRadiusY")
    $values.Add("rustyquest.projectionAreaCornerRadiusUv=$cornerRadius")
    $values.Add("rustyquest.projectionAreaOpacity=$areaOpacity")
    $values.Add("rustyquest.projectionBorderOpacity=$borderOpacity")
    $values.Add("rustyquest.projectionAlphaMode=$ProjectionAlphaMode")
    $values.Add("rustyquest.projectionAlphaScale=$alphaScale")
    $values.Add("rustyquest.projectionAlphaBias=$alphaBias")
    if (-not [double]::IsNaN($CameraPreviewFovYDegrees)) {
        $values.Add(("rustyquest.cameraPreviewFovYDegrees={0}" -f (Format-InvariantDouble -Value $CameraPreviewFovYDegrees)))
    }
    if (-not [double]::IsNaN($CameraPreviewOffsetYMeters)) {
        $values.Add(("rustyquest.cameraPreviewOffsetYMeters={0}" -f (Format-InvariantDouble -Value $CameraPreviewOffsetYMeters)))
    }
    if (-not [double]::IsNaN($CameraRawOverlayOverscan)) {
        $values.Add(("rustyquest.cameraRawOverlayOverscan={0}" -f (Format-InvariantDouble -Value $CameraRawOverlayOverscan)))
    }
...
function Get-BrokerH264Override {
    ...
    $values.Add("rustyquest.brokerH264SyntheticProjectionProfile=$BrokerH264SyntheticProjectionProfile")
    $values.Add("rustyquest.brokerH264ProjectionGeometryProfile=$projectionGeometryProfile")
...
function Invoke-QuestProfileMode {
    ...
    $preflightIssue = Get-StateReadinessIssue -State $stateBefore
    if ($preflightIssue) {
        $errorMessage = "Pre-mode headset state not ready: $preflightIssue"
...
function Invoke-MakepadMode {
    param(
        ...
        [double]$OffsetXUv = (Resolve-ProjectionAreaOffsetXUv -RendererValue $MakepadProjectionAreaOffsetXUv),
    ...
    $argList.Add("-ProjectionDepthMeters")
    $argList.Add((Format-InvariantDouble -Value $TargetProjectionDepthMeters))
    if (-not [double]::IsNaN($CameraPreviewFovYDegrees)) {
        $argList.Add("-CameraPreviewFovYDegrees")
        $argList.Add((Format-InvariantDouble -Value $CameraPreviewFovYDegrees))
    }
    if (-not [double]::IsNaN($CameraPreviewOffsetYMeters)) {
        $argList.Add("-CameraPreviewOffsetYMeters")
        $argList.Add((Format-InvariantDouble -Value $CameraPreviewOffsetYMeters))
    }
    if (-not [double]::IsNaN($CameraRawOverlayOverscan)) {
        $argList.Add("-CameraRawOverlayOverscan")
        $argList.Add((Format-InvariantDouble -Value $CameraRawOverlayOverscan))
    }
    $argList.Add("-XrRenderScale")
    $argList.Add((Format-InvariantDouble -Value $XrRenderScale))
    $argList.Add("-ProjectionAreaOffsetXUv")
    ...
    $argList.Add("-ProjectionAlphaMode")
    $argList.Add($ProjectionAlphaMode)
    $argList.Add("-ProjectionAlphaScale")
...
foreach ($modeId in $Mode) {
    ...
    switch ($modeId) {
        "vulkan-hwb-direct-camera2-raw" {
            $offsetXUv = Resolve-ModeProjectionAreaOffsetXUv `
                -RendererValue $VulkanProjectionAreaOffsetXUv `
                -ModeValue $VulkanDirectProjectionAreaOffsetXUv
            ...
            $projectionDepth = Resolve-VulkanProjectionDepthMeters -ModeValue $VulkanDirectProjectionDepthMeters
            ...
            $previewFovY = Resolve-VulkanOptionalDouble -CommonValue $CameraPreviewFovYDegrees -RendererValue $VulkanCameraPreviewFovYDegrees -ModeValue $VulkanDirectCameraPreviewFovYDegrees
            $previewOffsetY = $CameraPreviewOffsetYMeters
            $rawOverlayOverscan = Resolve-VulkanOptionalDouble -CommonValue $CameraRawOverlayOverscan -RendererValue $VulkanCameraRawOverlayOverscan -ModeValue $VulkanDirectCameraRawOverlayOverscan
        ...
        "vulkan-hwb-broker-h264-raw" {
            ...
            $offsetXUv = Resolve-BrokerModeProjectionAreaOffsetXUv `
                -RendererValue $VulkanProjectionAreaOffsetXUv `
                -ModeValue $VulkanBrokerProjectionAreaOffsetXUv `
                -FullFrameRendererValue $VulkanFullFrameBrokerProjectionAreaOffsetXUv
            ...
            $projectionDepth = Resolve-VulkanProjectionDepthMeters -ModeValue $VulkanBrokerProjectionDepthMeters
            ...
            $previewFovY = Resolve-VulkanOptionalDouble -CommonValue $CameraPreviewFovYDegrees -RendererValue $VulkanCameraPreviewFovYDegrees -ModeValue $VulkanBrokerCameraPreviewFovYDegrees
            $previewOffsetY = $CameraPreviewOffsetYMeters
            $rawOverlayOverscan = Resolve-VulkanOptionalDouble -CommonValue $CameraRawOverlayOverscan -RendererValue $VulkanCameraRawOverlayOverscan -ModeValue $VulkanBrokerCameraRawOverlayOverscan
        ...
        "gles-oes-direct-camera2-raw" {
            $offsetXUv = Resolve-ModeProjectionAreaOffsetXUv `
                -RendererValue $GlesProjectionAreaOffsetXUv `
                -ModeValue $GlesDirectProjectionAreaOffsetXUv
            $offsetYUv = Resolve-ModeProjectionAreaOffsetYUv `
            ...
            $projectionDepth = Resolve-GlesProjectionDepthMeters -ModeValue $GlesDirectProjectionDepthMeters
            $previewFovY = Resolve-CommonOptionalDouble -CommonValue $CameraPreviewFovYDegrees -RendererValue $GlesCameraPreviewFovYDegrees
            $previewOffsetY = Resolve-CommonOptionalDouble -CommonValue $CameraPreviewOffsetYMeters -RendererValue $GlesCameraPreviewOffsetYMeters
            $rawOverlayOverscan = Resolve-CommonOptionalDouble -CommonValue $CameraRawOverlayOverscan -RendererValue $GlesCameraRawOverlayOverscan
            Invoke-QuestProfileMode `
                -ModeId $modeId `
                ...
                -Apk $GlesApk `
                -InstallKey "gles" `
                -Override (Get-GlesProjectionBorderOverride -OffsetXUv $offsetXUv -OffsetYUv $offsetYUv -ScaleUv $scaleUv -TargetProjectionDepthMeters $projectionDepth -CameraPreviewFovYDegrees $previewFovY -CameraPreviewOffsetYMeters $previewOffsetY -CameraRawOverlayOverscan $rawOverlayOverscan)
        }
        "gles-oes-broker-h264-raw" {
            $sourceLabel = if ($BrokerH264SourceMode -eq "broker-synthetic") { "Broker synthetic H.264" } else { "Broker Camera2 -> H.264" }
            ...
            $offsetXUv = Resolve-BrokerModeProjectionAreaOffsetXUv `
                -RendererValue $GlesProjectionAreaOffsetXUv `
                -ModeValue $GlesBrokerProjectionAreaOffsetXUv `
                -FullFrameRendererValue $GlesFullFrameBrokerProjectionAreaOffsetXUv
            $offsetYUv = Resolve-BrokerModeProjectionAreaOffsetYUv `
                -RendererValue $GlesProjectionAreaOffsetYUv `
            ...
            $projectionDepth = Resolve-GlesProjectionDepthMeters -ModeValue $GlesBrokerProjectionDepthMeters
            $previewFovY = Resolve-CommonOptionalDouble -CommonValue $CameraPreviewFovYDegrees -RendererValue $GlesCameraPreviewFovYDegrees
            $previewOffsetY = Resolve-CommonOptionalDouble -CommonValue $CameraPreviewOffsetYMeters -RendererValue $GlesCameraPreviewOffsetYMeters
            $rawOverlayOverscan = Resolve-CommonOptionalDouble -CommonValue $CameraRawOverlayOverscan -RendererValue $GlesCameraRawOverlayOverscan
            Invoke-QuestProfileMode `
                ...
                -Apk $GlesApk `
                -InstallKey "gles" `
                -Override (Join-OverrideValues -Values @((Get-BrokerH264Override), (Get-GlesProjectionBorderOverride -OffsetXUv $offsetXUv -OffsetYUv $offsetYUv -ScaleUv $scaleUv -TargetProjectionDepthMeters $projectionDepth -CameraPreviewFovYDegrees $previewFovY -CameraPreviewOffsetYMeters $previewOffsetY -CameraRawOverlayOverscan $rawOverlayOverscan)))
        }
        "makepad-cpuyuv-direct-camera2-raw" {
            $offsetXUv = Resolve-ModeProjectionAreaOffsetXUv `
                -RendererValue $MakepadProjectionAreaOffsetXUv `
                -ModeValue $MakepadDirectProjectionAreaOffsetXUv
            $offsetYUv = Resolve-ModeProjectionAreaOffsetYUv `
            ...
            $projectionDepth = Resolve-MakepadProjectionDepthMeters -ModeValue $MakepadDirectProjectionDepthMeters
            $xrRenderScale = Resolve-MakepadXrRenderScale -ModeValue $MakepadDirectXrRenderScale
            $previewFovY = Resolve-CommonOptionalDouble -CommonValue $CameraPreviewFovYDegrees -RendererValue $MakepadCameraPreviewFovYDegrees
            $previewOffsetY = Resolve-CommonOptionalDouble -CommonValue $CameraPreviewOffsetYMeters -RendererValue $MakepadCameraPreviewOffsetYMeters
            $rawOverlayOverscan = Resolve-CommonOptionalDouble -CommonValue $CameraRawOverlayOverscan -RendererValue $MakepadCameraRawOverlayOverscan
            Invoke-MakepadMode `
                -ModeId $modeId `
                ...
                -TargetProjectionDepthMeters $projectionDepth `
                -XrRenderScale $xrRenderScale `
                -CameraPreviewFovYDegrees $previewFovY `
                -CameraPreviewOffsetYMeters $previewOffsetY `
                -CameraRawOverlayOverscan $rawOverlayOverscan
        }
        "makepad-cpuyuv-broker-h264-raw" {
            $sourceLabel = if ($BrokerH264SourceMode -eq "broker-synthetic") { "Broker synthetic H.264" } else { "Broker Camera2 -> H.264" }
            $offsetXUv = Resolve-BrokerModeProjectionAreaOffsetXUv `
                -RendererValue $MakepadProjectionAreaOffsetXUv `
                -ModeValue $MakepadBrokerProjectionAreaOffsetXUv `
                -FullFrameRendererValue $MakepadFullFrameBrokerProjectionAreaOffsetXUv
            $offsetYUv = Resolve-BrokerModeProjectionAreaOffsetYUv `
                -RendererValue $MakepadProjectionAreaOffsetYUv `
            ...
            $projectionDepth = Resolve-MakepadProjectionDepthMeters -ModeValue $MakepadBrokerProjectionDepthMeters
...
$lines.Add(("- Projection area offset X UV: ``{0}``" -f (Format-InvariantDouble -Value $ProjectionAreaOffsetXUv)))
...
$lines.Add(("- GL/OES broker projection area offset X UV: ``{0}``" -f (Format-InvariantDouble -Value (Resolve-ModeProjectionAreaOffsetXUv -RendererValue $GlesProjectionAreaOffsetXUv -ModeValue $GlesBrokerProjectionAreaOffsetXUv))))
$lines.Add(("- Makepad CPU-YUV direct projection area offset X UV: ``{0}``" -f (Format-InvariantDouble -Value (Resolve-ModeProjectionAreaOffsetXUv -RendererValue $MakepadProjectionAreaOffsetXUv -ModeValue $MakepadDirectProjectionAreaOffsetXUv))))
$lines.Add(("- Makepad CPU-YUV broker projection area offset X UV: ``{0}``" -f (Format-InvariantDouble -Value (Resolve-ModeProjectionAreaOffsetXUv -RendererValue $MakepadProjectionAreaOffsetXUv -ModeValue $MakepadBrokerProjectionAreaOffsetXUv))))
...
$fullFrameBrokerOffsetXLabel = if ([double]::IsNaN($FullFrameBrokerProjectionAreaOffsetXUv)) {
    "not set"
}
else {
    Format-InvariantDouble -Value $FullFrameBrokerProjectionAreaOffsetXUv
}
$lines.Add(("- Full-frame diagnostic broker projection area offset X UV: ``{0}``" -f $fullFrameBrokerOffsetXLabel))
$lines.Add(("- Vulkan/HWB full-frame broker projection area offset X UV: ``{0}``" -f (Format-InvariantDouble -Value (Resolve-BrokerModeProjectionAreaOffsetXUv -RendererValue $VulkanProjectionAreaOffsetXUv -ModeValue $VulkanBrokerProjectionAreaOffsetXUv -FullFrameRendererValue $VulkanFullFrameBrokerProjectionAreaOffsetXUv))))
$lines.Add(("- GL/OES full-frame broker projection area offset X UV: ``{0}``" -f (Format-InvariantDouble -Value (Resolve-BrokerModeProjectionAreaOffsetXUv -RendererValue $GlesProjectionAreaOffsetXUv -ModeValue $GlesBrokerProjectionAreaOffsetXUv -FullFrameRendererValue $GlesFullFrameBrokerProjectionAreaOffsetXUv))))
$lines.Add(("- Makepad CPU-YUV full-frame broker projection area offset X UV: ``{0}``" -f (Format-InvariantDouble -Value (Resolve-BrokerModeProjectionAreaOffsetXUv -RendererValue $MakepadProjectionAreaOffsetXUv -ModeValue $MakepadBrokerProjectionAreaOffsetXUv -FullFrameRendererValue $MakepadFullFrameBrokerProjectionAreaOffsetXUv))))
...
$lines.Add(("- Shared preview FOV Y / offset Y meters / raw overscan: ``{0}``, ``{1}``, ``{2}``" -f (Format-OptionalInvariantDouble -Value $CameraPreviewFovYDegrees), (Format-OptionalInvariantDouble -Value $CameraPreviewOffsetYMeters), (Format-OptionalInvariantDouble -Value $CameraRawOverlayOverscan)))
$lines.Add(("- GL/OES preview FOV Y / offset Y meters / raw overscan: ``{0}``, ``{1}``, ``{2}``" -f (Format-OptionalInvariantDouble -Value (Resolve-CommonOptionalDouble -CommonValue $CameraPreviewFovYDegrees -RendererValue $GlesCameraPreviewFovYDegrees)), (Format-OptionalInvariantDouble -Value (Resolve-CommonOptionalDouble -CommonValue $CameraPreviewOffsetYMeters -RendererValue $GlesCameraPreviewOffsetYMeters)), (Format-OptionalInvariantDouble -Value (Resolve-CommonOptionalDouble -CommonValue $CameraRawOverlayOverscan -RendererValue $GlesCameraRawOverlayOverscan))))
$lines.Add(("- Makepad CPU-YUV preview FOV Y / offset Y meters / raw overscan: ``{0}``, ``{1}``, ``{2}``" -f (Format-OptionalInvariantDouble -Value (Resolve-CommonOptionalDouble -CommonValue $CameraPreviewFovYDegrees -RendererValue $MakepadCameraPreviewFovYDegrees)), (Format-OptionalInvariantDouble -Value (Resolve-CommonOptionalDouble -CommonValue $CameraPreviewOffsetYMeters -RendererValue $MakepadCameraPreviewOffsetYMeters)), (Format-OptionalInvariantDouble -Value (Resolve-CommonOptionalDouble -CommonValue $CameraRawOverlayOverscan -RendererValue $MakepadCameraRawOverlayOverscan))))
...
$lines.Add(("- Vulkan/HWB direct preview FOV Y / offset Y meters / raw overscan / full-view overscan: ``{0}``, ``{1}``, ``{2}``, ``{3}``" -f (Format-OptionalInvariantDouble -Value (Resolve-VulkanOptionalDouble -CommonValue $CameraPreviewFovYDegrees -RendererValue $VulkanCameraPreviewFovYDegrees -ModeValue $VulkanDirectCameraPreviewFovYDegrees)), (Format-OptionalInvariantDouble -Value $CameraPreviewOffsetYMeters), (Format-OptionalInvariantDouble -Value (Resolve-VulkanOptionalDouble -CommonValue $CameraRawOverlayOverscan -RendererValue $VulkanCameraRawOverlayOverscan -ModeValue $VulkanDirectCameraRawOverlayOverscan)), (Format-OptionalInvariantDouble -Value (Resolve-VulkanOptionalDouble -RendererValue $VulkanCameraFullViewOverlayOverscan -ModeValue $VulkanDirectCameraFullViewOverlayOverscan))))
$lines.Add(("- Vulkan/HWB broker preview FOV Y / offset Y meters / raw overscan / full-view overscan: ``{0}``, ``{1}``, ``{2}``, ``{3}``" -f (Format-OptionalInvariantDouble -Value (Resolve-VulkanOptionalDouble -CommonValue $CameraPreviewFovYDegrees -RendererValue $VulkanCameraPreviewFovYDegrees -ModeValue $VulkanBrokerCameraPreviewFovYDegrees)), (Format-OptionalInvariantDouble -Value $CameraPreviewOffsetYMeters), (Format-OptionalInvariantDouble -Value (Resolve-VulkanOptionalDouble -CommonValue $CameraRawOverlayOverscan -RendererValue $VulkanCameraRawOverlayOverscan -ModeValue $VulkanBrokerCameraRawOverlayOverscan)), (Format-OptionalInvariantDouble -Value (Resolve-VulkanOptionalDouble -RendererValue $VulkanCameraFullViewOverlayOverscan -ModeValue $VulkanBrokerCameraFullViewOverlayOverscan))))
...
$lines.Add(("- Projection alpha mode/scale/bias: ``{0}``, ``{1}``, ``{2}``" -f $ProjectionAlphaMode, (Format-InvariantDouble -Value $ProjectionAlphaScale), (Format-InvariantDouble -Value $ProjectionAlphaBias)))
