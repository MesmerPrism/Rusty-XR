...
public final class MediaProjectionStreamService extends Service {
    ...
    private static final String TAG = "RustyXrMediaStream";
    private static final String CHANNEL_ID = "rusty_xr_media_projection";
    private static final int NOTIFICATION_ID = 8703;
    ...
    private static native void nativeMediaProjectionEvent(String eventJson);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (manager == null || resultData == null || resultCode == 0) {
            ...
            sendNativeEvent("mediaProjectionMissingResult");
            stopSelf();
            return START_NOT_STICKY;
        }

        captureThread = new HandlerThread("RustyXrMediaProjection");
        captureThread.start();
    ...
    public void onDestroy() {
        closeQuietly();
        sendNativeEvent("mediaProjectionStreamStopped");
        super.onDestroy();
    }
    ...
    private void startCapture(
        ...
        try {
            ...
            sendNativeEvent("mediaProjectionSocketConnected");

            mediaProjection = manager.getMediaProjection(resultCode, resultData);
            if (mediaProjection == null) {
                Log.e(TAG, "MediaProjection was not created");
                sendNativeEvent("mediaProjectionCreateFailed");
                stopSelf();
                return;
            }
            mediaProjection.registerCallback(new MediaProjection.Callback() {
                ...
                public void onStop() {
                    ...
                    sendNativeEvent("mediaProjectionStoppedBySystem");
            ...
            sendNativeEvent("mediaProjectionStreamStarted");
        } catch (IOException error) {
            Log.e(TAG, "Could not connect to media receiver", error);
            sendNativeEvent("mediaProjectionSocketFailed");
            stopSelf();
        } catch (RuntimeException error) {
            Log.e(TAG, "MediaProjection stream setup failed", error);
            sendNativeEvent("mediaProjectionStreamSetupFailed");
            stopSelf();
        }
    }
    ...
    private Notification createNotification() {
        ...
        return builder
            .setContentTitle("Rusty XR media capture")
            .setContentText("Streaming display-composite frames to the paired Windows receiver.")
    ...
    private static void sendNativeEvent(String name) {
        try {
            nativeMediaProjectionEvent("{\"event\":\"" + name + "\",\"timeNs\":" + System.nanoTime() + "}");
        } catch (UnsatisfiedLinkError error) {
            Log.w(TAG, "Native media event bridge unavailable: " + name);
        }
    }
}
