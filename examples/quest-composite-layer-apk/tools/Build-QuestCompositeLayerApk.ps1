<#
.SYNOPSIS
    Builds the public Rusty XR Quest composite-layer debug APK.
#>
[CmdletBinding()]
param(
...
$debugKeyRoot = Join-Path $env:LOCALAPPDATA 'RustyXrDebugKeystores\quest-composite-layer'
...
function Resolve-Glslc {
    ...
    $existing = [Environment]::GetEnvironmentVariable('GLSLC')
    if (-not [string]::IsNullOrWhiteSpace($existing) -and (Test-Path $existing)) {
        return (Resolve-Path $existing).Path
    }

    $candidate = Join-Path $NdkRoot 'shader-tools\windows-x86_64\glslc.exe'
    if (Test-Path $candidate) {
        return $candidate
    }

    $match = Get-ChildItem -LiteralPath $NdkRoot -Recurse -File -Filter 'glslc.exe' -ErrorAction SilentlyContinue |
        Select-Object -First 1
    if ($null -ne $match) {
        return $match.FullName
    }

    throw "Could not find glslc under $NdkRoot. Set GLSLC to an Android Vulkan shader compiler."
}
...
$glslc = Resolve-Glslc -NdkRoot $ndkRoot

foreach ($tool in @($androidJar, $linker, $javac, $jar, $keytool, $aapt2, $d8, $zipalign, $apksigner, $openXrLoader, $libCpp, $glslc)) {
...
$nativeLibrary = Join-Path $repoRoot 'target\aarch64-linux-android\release\librusty_xr_quest_composite_native.so'
