...
The active slice is S90. S79 matched the public target's display-to-source
...
step does not silently fall back to source index. No further source-only
projection change is being claimed before the runtime camera-ID binding and
headset parallax checks run.
...
| S90 | Camera-ID source binding parity | Makepad fork patch, fork validation and push, lockfile update, source patch, standalone source validation, fresh release APK build, native-library string check, device-gate parser update, public-boundary scan, public push, static target diff, and focused host unit tests. | Build/string-ready, device gate pending. The target diff showed a likely Makepad-only ambiguity: the public target selects the stereo pair by Camera2 pose X and reports display camera IDs, while Makepad Android descriptors previously named all back cameras identically and the example paired Makepad video choices by source index. The fork now appends a stable `cameraId=` token to Android video descriptors. The example now carries `leftCameraId` / `rightCameraId` through `StereoProjectionPlan`, orders the Camera2 stereo pair by pose X with the old index-scored selection as fallback, parses Makepad descriptor camera IDs, binds Makepad video streams by ID when possible, and reports `sourceBindingMode`. Standalone `cargo fmt --check` and `cargo check` pass, with only the known duplicate Makepad `bitflags` warning. Focused host unit tests now cover descriptor `cameraId=` parsing, size-preferred camera-ID selection, and camera-ID pair binding overriding misleading source indices. A fresh release APK built, the native library contains S90, `cameraId=`, and pose-X source-selection strings, and stale S89/S88/S87/S86 path strings are absent. The source state is pushed. A static diff against the validated public fast `0.75` target found no remaining source-only mismatch in source ordering, runtime view/FOV inputs, homography helper chain, projection scale, preview FOV, raw overscan, or `left-right` source-eye mapping. Runtime `sourceBindingMode=camera-id` still requires logcat because it is formatted from the selected descriptor at run time. | Run S90 once ADB transport is available. Require `s90CameraIdSourceBinding=true`, `sourceBindingMode=camera-id`, active XR, runtime-view/homography markers, byte-distinct screenshots, and fault-clean logs. If those pass, operator visual inspection can decide whether the parallax gap is closed; if not, continue with exact target projection math. |
