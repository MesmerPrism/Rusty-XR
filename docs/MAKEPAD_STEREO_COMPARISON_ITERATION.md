...
| S105 | Hotloadable horizontal alignment control | Public target shader diff, Makepad source patch, Android-property hotload adapter, standalone source validation, direct generated-XR activity gate, public-target screenshot capture, Makepad strength sweep screenshots, and helper script for ADB/MCP-driven tuning. | In progress. S105 keeps S103's full submitted surface, in-shader camera window, matte, border, rotation, aspect, and launch lane. The automatic X correction now uses the Rusty XR-style `screen_to_camera` homography at submitted-surface center. S105 also polls `debug.rustyxr` Android properties while the app is running and applies additive manual UV offsets per eye; `examples/makepad-q2q-camera-shell/tools/Send-MakepadQ2QHorizontalOffset.ps1` sets those properties over ADB. Host validation passed with the known Makepad duplicate-package warning. The reset device run reached active XR, emitted the S105 and hotload markers, captured six byte-distinct frames, and stayed GPU-fault/fatal clean, but operator review reported severe horizontal misalignment plus left/right edge striping. `Strength=0` removed the striping class but put the views too far apart. A raw-screenshot comparison against the public fast `0.75` target then swept `0`, `0.25`, `0.5`, `0.75`, `0.85`, `0.37`, `0.41`, and `0.425`; feature matching put `0.425` closest to the public target's normalized left/right disparity. The source default now uses `0.425`, with hotload still available for live override. | Get operator headset review of `Strength=0.425`. If visual alignment is accepted, keep this as the next Makepad default and continue into valid-region/edge-stripe cleanup plus performance comparison. If it is still visually off, use the same screenshot matcher plus manual `LeftUv`/`RightUv` offsets rather than changing the full-layer/in-shader-window architecture. |
