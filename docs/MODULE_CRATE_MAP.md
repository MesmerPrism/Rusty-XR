...
| Crate or module | Role | Notes |
| --- | --- | --- |
| `rusty-xr-broker-model` | Broker command envelopes, acknowledgements, stream manifests, sample headers, replay records, session manifests, transport offers/answers, packet descriptors, clock snapshots, and stream health. | Transport and codec implementation stay outside the model crate. |
