...
| Crate or module | Role | Notes |
| --- | --- | --- |
| `rusty-xr-broker-model` | Broker command envelopes, acknowledgements, stream manifests, sample headers, replay records, session manifests, transport offers/answers, ZeroMQ bridge manifests, packet descriptors, clock snapshots, stream health, broker module manifests/runtime state, broker panel descriptors, stream registry snapshots, and lease-aware command authority contracts. | Transport, socket, UI framework, dynamic module loading, device mutation, and codec implementation stay outside the model crate. |
