...
| Broker HTTP/WebSocket API | Example-owned | Runtime status, clock, stream, command, and kiosk control-plane reporting. | `/status`, `/kiosk/status`, `/clock/now`, `/clock/health`, WebSocket commands, broker streams. |
